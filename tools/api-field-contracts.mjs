...
export const fieldContracts = {
  "AuthController.capabilities": record(null, null, object({ realm: { const: "global" }, defaultLocale: { const: "en" }, supportedLocales: array(globalLocale), registration: object({ email: boolean, sms: boolean }, ["email", "sms"]), recovery: object({ email: boolean, sms: boolean }, ["email", "sms"]), smsCountries: array(string), verification: object({ codeLength: integer, expiresIn: integer, retryAfter: integer }), consentVersion: { type: ["string", "null"] }, legal: { oneOf: [{ type: "null" }, object({ userAgreement: legalReference, privacyPolicy: legalReference }, ["userAgreement", "privacyPolicy"])] } }), { realm: "global", defaultLocale: "en", supportedLocales: globalLocale.enum, registration: { email: false, sms: false }, recovery: { email: false, sms: false }, smsCountries: [], verification: { codeLength: 6, expiresIn: 300, retryAfter: 60 }, consentVersion: null, legal: null }, globalAuthSource),
