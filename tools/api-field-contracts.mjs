...
export const fieldContracts = {
  ...
  "CommerceController.markets": record(null, null, object({ markets: array(object({ countryCode: string, currency: string, currencyExponent: integer, commerceEnabled: boolean, paymentChannels: array(string) })) }, ["markets"]), { markets: [{ countryCode: "CN", currency: "CNY", currencyExponent: 2, commerceEnabled: true, paymentChannels: [] }] }, "apps/api/src/commerce/global-commerce-policy.ts; apps/api/src/commerce/commerce-store.service.ts"),
  ...
  "AdminController.importCommerceProductBySku": record(
    object({ sku: { type: "string", minLength: 1, maxLength: 100, pattern: "^[^,\\r\\n]+$" } }, ["sku"]),
    { sku: "ERP-SKU-001" },
    object({ id, source: { const: "ERP" }, erpItemId: string, name: string, skus: array(object({ id, erpSkuId: string, stock: integer, salePriceCents: integer }, ["id", "erpSkuId", "stock", "salePriceCents"])), erpLookup: object({ requestedSku: string, fetchedAt: { type: "string", format: "date-time" }, product: object({}), inventory: object({}) }, ["requestedSku", "fetchedAt", "product", "inventory"]) }, ["id", "source", "erpItemId", "name", "skus", "erpLookup"]),
