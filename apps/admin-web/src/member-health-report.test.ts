...
describe("member AI health report panel", () => {
  it("shows server gates and never creates when generation is unavailable", async () => {
    const h = harness(); h.api.get.mockResolvedValueOnce(response({ ...ready, canGenerate: false, reasons: [{ code: "ai_unconfigured", message: "AI 服务未配置" }] }));
    await h.checkAvailability(); await h.generateReport();
    expect(h.availability.value.reasons).toHaveLength(1); expect(h.api.post).not.toHaveBeenCalled(); expect(h.confirm).not.toHaveBeenCalled();
  });
  ...
  it("states the audited super-admin boundary without rendering the removed member-consent gate", () => {
