...
export const integrationDefinitions: IntegrationDefinition[] = [
  ...
  { key: 'ai', title: 'AI 问答与报告', group: '运营与系统', short: 'AI', purpose: '为问答和健康报告提供 AI 服务。', prepare: '准备兼容 Chat Completions 的服务地址、模型 ID 和 API Key；服务地址通常包含 /v1，不要再附加 /chat/completions。', note: '所有支持的服务均通过兼容接口调用。保存不会生成报告或产生模型调用费用。', fields: [provider('openai_compatible', '兼容 Chat Completions 的服务'), url('baseUrl', 'AI 服务地址', '例如服务商提供的 https://服务域名/v1。', { required: true }), p('model', '模型 ID', '填写服务商模型列表返回的实际 ID（例如 glm-5.3-flash），不能填写服务商品牌名。', { required: true }), s('apiKey', '访问密钥（API Key）', '从已开通的模型服务取得。')] },
...
export function integrationPayload(row: IntegrationRow, definition: IntegrationDefinition, draft: IntegrationDraft): { state: string; publicConfig: Record<string, unknown>; secrets?: Record<string, string> } {
  ...
  for (const f of activeFields) {
    ...
    const persistSelectedProvider = f.key === 'provider' && get(publicConfig, f.key) == null && Boolean(value) && (draft.state === 'CONFIGURED' || draft.replaceSecrets);
    if (raw === initial.values[f.key] && !persistSelectedProvider) continue;
