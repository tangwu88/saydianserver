<template>
  <DesktopHeader />
  <view class="saydian-app-surface auth-surface"
...
const isWecom = /wxwork/i.test(userAgent),
  isWechat = /micromessenger/i.test(userAgent) && !isWecom;
...
async function loadCapabilities() {
  ...
  documents.value = {};
  consent.value = { version: "", locale: "en" };
...
async function initialize(handleCallback: boolean) {
  ...
  try {
    ...
    accepted.value = true;
    busy.value = true;
    const response: any = await api("/auth/wechat/h5/login", {
      method: "POST",
      data: {
        code: callback.code,
        state: context.state,
...
function changeContact(next: "email" | "sms") {
  if (busy.value) return;
  contactMode.value = next;
  identifier.value = "";
  resetChallenge();
  error.value = "";
}
function startCountdown(seconds: number) {
  resendAt = Date.now() + Math.max(1, Math.min(86400, Math.ceil(seconds))) * 1000;
  countdown.value = Math.ceil((resendAt - Date.now()) / 1000);
  if (timer) clearInterval(timer);
  timer = setInterval(() => {
    countdown.value = Math.max(0, Math.ceil((resendAt - Date.now()) / 1000));
    if (!countdown.value && timer) clearInterval(timer);
  }, 1000);
}
function requireBinding() {
  if (!bindTicket.value || Date.now() >= bindExpiresAt.value) {
    cancelBinding();
    throw authUiError("微信登录已过期，请重新登录。");
  }
  if (bindingSession !== mallOAuthSessionStamp()) {
    cancelBinding();
    throw authUiError("账号已切换，请重新登录。");
  }
...
async function requestPhoneCode(recipient: string, testOnly: boolean) {
  ...
  const response: any = await api("/auth/wechat/h5/phone-code", {
    method: "POST",
    data: {
      bindTicket: ticket,
      identifier: recipient,
      locale: consent.value.locale,
      ...(testOnly ? { expectedMode: "test" } : {}),
    },
  });
  if (!active) return;
  requireBinding();
...
async function officialLogin() {
  ...
  try {
    ...
    const requestSession = mallOAuthSessionStamp(),
      agreed = { ...consent.value };
    const returnTo = safeMallRoute(mallStorage.get("saidian-post-login-route"));
    const response: any = await api("/auth/wechat/h5/authorize-url", {
      method: "POST",
      data: {
        returnTo,
    ...
    const context: OAuthContext = {
      ...
      consentVersion: agreed.version,
      locale: agreed.locale,
      expiresAt: Date.now() + Math.min(response.expiresIn * 1000, OAUTH_TTL),
      returnTo,
      sessionStamp: requestSession,
    };
...
async function showError(cause: unknown) {
  ...
  if (["consent_outdated", "consent_required", "legal_unavailable"].includes(key)) {
    ...
    try {
      ...
      /* Original friendly consent error stays visible. */
    }
  }
