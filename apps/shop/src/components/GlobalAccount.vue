<template>
  ...
  <view class="saydian-app-surface account-surface"><view class="account-content">
    ...
    <view class="app-panel shopping-menu" aria-label="我的订单">
      <button @click="go('/pages/orders/index')">全部订单</button><button @click="go('/pages/orders/index?status=PENDING_PAYMENT')">待付款</button><button @click="go('/pages/orders/index?status=SHIPPED')">待收货</button><button @click="go('/pages/orders/index?status=AFTER_SALE')">售后</button>
    </view>
