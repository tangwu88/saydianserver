<template><DesktopHeader/><view class="page"><view class="container checkout-layout"><view>
  ...
  <view class="card block benefits-card"><button class="benefits-toggle" :disabled="submitting" :aria-expanded="showBenefits" @click="showBenefits=!showBenefits"><view><b>优惠、积分与备注</b><text>{{ benefitsSummary }}</text></view><text>{{ showBenefits ? '收起' : '展开' }} ›</text></button><view v-if="showBenefits" class="benefits-body">
  <view class="coupon-code"><input v-model="couponCode" class="input" maxlength="32" placeholder="输入优惠码" :disabled="submitting||couponBusy" confirm-type="done" @confirm="redeemCouponCode"/><button class="outline-btn" :loading="couponBusy" :disabled="submitting||couponBusy||!couponCode.trim()" @click="redeemCouponCode">兑换</button></view><text v-if="couponError" class="coupon-error" role="alert">{{ couponError }}</text>
  <view class="field"><text class="field-label">我的优惠券</text><picker class="coupon-picker" :range="couponLabels" :disabled="submitting||couponBusy" @change="selectCoupon"><view>{{ selectedCoupon?.coupon?.name || "不使用优惠券" }} ›</view></picker></view><view class="field"><text>积分可抵扣金额</text><b>{{ quote?.availablePointCents == null ? "未获取" : money(quote.availablePointCents) }}</b></view>
  ...
  <button class="optional-toggle" :disabled="submitting" @click="showRemark=!showRemark">{{ showRemark ? '收起订单备注' : '添加订单备注（选填）' }} ›</button><label v-if="showRemark" class="form-label">订单备注<input v-model="remark" :disabled="submitting" class="input" maxlength="500" placeholder="如有配送要求，请在这里填写"/></label></view></view>
...
const items=ref<any[]>([]),addresses=ref<any[]>([]),address=ref<any>(),coupons=ref<any[]>([]),selectedCoupon=ref<any>(),quote=ref<any>(),capabilities=ref<any>(),remark=ref(""),showRemark=ref(false),showBenefits=ref(false),pointAmount=ref("0.00"),couponCode=ref(""),couponError=ref(""),couponBusy=ref(false),submitting=ref(false),quoting=ref(false),error=ref("");
let revision=0;
...
const benefitsSummary=computed(()=>{const saved=Number(quote.value?.couponDiscountCents||0)+Number(quote.value?.pointDiscountCents||0);if(saved>0)return `已优惠 ${money(saved)}`;if(remark.value.trim())return '已填写订单备注';return '有优惠码或积分时可使用';});
...
<style scoped>.field-label{flex:none;white-space:nowrap}.coupon-picker{min-width:0;flex:1;text-align:right;overflow-wrap:anywhere;line-height:1.6;padding:12px 0}.benefits-card{padding:0;overflow:hidden}.benefits-toggle{width:100%;min-height:76px;margin:0;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;gap:14px;text-align:left;background:#fff;color:var(--ink);font-size:14px;line-height:1.5}.benefits-toggle::after{border:0}.benefits-toggle view{display:grid;gap:3px}.benefits-toggle view>text{color:var(--muted);font-size:13px}.benefits-toggle>text{color:var(--green);white-space:nowrap}.benefits-body{padding:0 20px 18px;border-top:1px solid var(--line)}.coupon-code{display:grid;grid-template-columns:minmax(0,1fr) 88px;gap:10px;align-items:center;padding:16px 0}.coupon-code .input{min-width:0}.coupon-code button{min-height:46px;margin:0;padding:8px 12px}.coupon-error{display:block;margin:-6px 0 10px;color:#be092d;font-size:13px}.optional-toggle{width:100%;margin:8px 0 0;padding:10px 0;min-height:44px;text-align:left;background:transparent;color:var(--green);font-size:14px;line-height:1.5}.optional-toggle::after{border:0}</style>
