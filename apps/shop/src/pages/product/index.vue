...
async function buildSharePoster():Promise<string>{
  ...
  const logo=await loadPosterImage(saidianBrandLogo);if(logo)drawContain(ctx,logo,72,62,270,64);else{ctx.fillStyle='#d20b27';ctx.font='700 34px sans-serif';ctx.fillText('SAYDIAN',72,104);}ctx.fillStyle='#111827';ctx.font='700 42px sans-serif';drawWrappedText(ctx,String(product.value.displayName||product.value.name||'赛电商品'),72,170,606,56,2);
