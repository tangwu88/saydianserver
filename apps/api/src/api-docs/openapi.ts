...
function rawResponseContract(
  route: Json,
): { mediaTypes: string[]; schema: Json; example?: unknown } | undefined {
  const key = String(route.routeKey ?? route.key);
  if (key === "HealthReportsController.export")
    return {
      mediaTypes: ["application/pdf"],
      schema: { type: "string", format: "binary" },
    };
  if (key === "FilesController.download")
    return {
      ...
      schema: {
        ...
        format: "binary",
        description:
          "原始头像文件字节；Content-Type 取决于已保存文件，不含JSON包裹",
      },
    };
  if (
    [
    ...
    return {
      ...
      schema: {
        ...
        format: "binary",
        description:
          "经身份与文件归属校验的私有售后图片字节；不含JSON包裹，后台读取须完成审计",
      },
    };
  }
  if (
...
export function generateOpenApi(items: Json[], version: string) {
  ...
  for (const route of items) {
    ...
    for (const match of path.matchAll(/\{([^}]+)\}/g)) {
      parameters.push({
        ...
        required: true,
        schema: { type: "string" },
        description: "资源标识；旧接口保留来源整数ID，新接口通常为UUID",
      });
    }
    for (const parameter of (route.parameters ?? []) as Json[]) {
      ...
      parameters.push({
        name: parameter.name,
        in: parameter.in,
        required: documented?.required ?? !parameter.optional,
    ...
    for (const [name, documented] of Object.entries(
      contract.query ?? {},
    ...
    const successStatus = String(
      route.successStatus ?? (method === "post" ? 201 : 200),
    );
    const responseExample = raw
      ? raw.example
      : contract.responseExample === null ||
          contract.responseExample === undefined
        ? undefined
        : envelope
          ? {
              code: 200,
              message: "OK",
              data: contract.responseExample,
              ...(!legacy
                ? { timestamp: 1_788_825_600, requestId: "contract-example" }
                : {}),
            }
          : contract.responseExample;
    const responses: Json = {
      [successStatus]: {
        description: `成功：${route.response ?? "见业务说明"}`,
        content: Object.fromEntries(
          (raw?.mediaTypes ?? ["application/json"]).map((mediaType) => [
            mediaType,
            {
    ...
    if (legacy) {
      ...
      responses["200"] = {
        description:
          "旧响应：必须检查业务code；失败也使用HTTP 200，不能只检查HTTP状态",
        content: { "application/json": { schema } },
      };
    } else {
      for (const status of [
        400,
        ...(route.auth === "public" ? [] : [401, 403]),
        404,
        409,
        429,
        503,
    ...
    const hasBody = (route.parameters ?? []).some((item: Json) =>
      ["body", "file"].includes(item.in),
    );
    const operation: Json = {
      operationId: String(route.routeKey ?? route.key).replaceAll(".", "_"),
      summary: route.title ?? route.summary,
      description: `${route.summary ?? ""}\n\n${contract.note ?? "字段尚待复核"}`,
      tags: route.tags?.length ? route.tags : [String(route.auth)],
        ...
        content: {
          [contract.contentType ?? "application/json"]: {
            schema: contract.requestSchema ?? {
              ...
              description: "未复核字段，不可据此判定兼容",
            },
            ...(contract.requestExample
              ? { example: contract.requestExample }
              : {}),
          },
        },
      };
  ...
  return {
    openapi: "3.1.0",
    info: { title: "Saydian赛电 API", version },
    servers: [{ url: "/" }],
    paths,
    components: {
      securitySchemes: {
...
export function documentedExample(route: Json) {
  ...
  const raw = rawResponseContract(route);
  const authHeader =
    route.auth === "public" ? "" : ' -H "Authorization: Bearer <ACCESS_TOKEN>"';
  const needsBody = (route.parameters ?? []).some((item: Json) =>
    ["body", "file"].includes(item.in),
  );
  const hasExample =
    contract.requestExample !== null && contract.requestExample !== undefined;
  ...
  const query = Object.entries(contract.query ?? {})
    .map(
      ([key, value]) =>
        `${encodeURIComponent(key)}=${encodeURIComponent(String((value as Json).example))}`,
    )
    .join("&");
  const url = `{{baseUrl}}${openApiPath(route.path)}${query ? `?${query}` : ""}`;
  const idempotency = /Idempotency-Key/i.test(route.request)
    ? ' -H "Idempotency-Key: <UNIQUE_OPERATION_KEY>"'
    : "";
  return {
    场景: route.summary,
    字段状态: contract.status ?? "unreviewed",
    curl:
      body === null
        ? null
        : `curl -X ${route.method} "${url}"${authHeader}${idempotency}${body}`,
    请求体: contract.requestExample ?? null,
    请求Schema: contract.requestSchema ?? null,
    返回类型: raw?.mediaTypes ?? ["application/json"],
    返回Schema: raw?.schema ?? contract.responseSchema ?? null,
    返回示例: raw ? (raw.example ?? null) : (contract.responseExample ?? null),
    请求说明: route.request,
    预期返回: route.response,
    依赖: route.dependency,
    说明:
      body === null
        ? "此接口尚无经过复核的字段示例，不能把空对象当作有效请求。"
        : "合成示例。curl使用POSIX单引号语法；替换baseUrl、路径参数和尖括号占位符后在测试环境使用。PowerShell可用请求体配合Invoke-RestMethod；禁止用真实密码或健康数据填写发布文档。",
  };
}
