...
describe("international member contact verification administration", () => {
  ...
  it("updates member details and verification atomically, revokes sessions and omits PII from audit", async () => {
    ...
    expect(h.tx.user.updateMany).toHaveBeenCalledWith({
      ...
      data: expect.objectContaining({
        ...
        emailVerifiedAt: new Date("2026-09-10T00:00:00.000Z"),
        avatarUrl: "https://cdn.example.invalid/updated.png",
        gender: "MALE",
        birthday: new Date("1991-03-04T00:00:00.000Z"),
        heightCm: expect.anything(),
        weightKg: expect.anything(),
      }),
    });
    expect(h.tx.userSession.updateMany).toHaveBeenCalledWith({
      where: { userId: h.current()!.id, revokedAt: null },
  ...
  it("lets a super administrator reset a member password, revokes sessions and never audits the password", async () => {
    ...
    await h.service.updateMemberProfile(superAdmin, h.current()!.id, "password-reset", {
      nickname: "Synthetic member",
      mobile: "+8613812348888",
      email: "member@example.invalid",
      status: "ACTIVE",
    ...
    const update = h.tx.user.updateMany.mock.calls[0]?.[0];
    ...
    const audit = JSON.stringify(h.auditLog.create.mock.calls[0]?.[0]);
    expect(audit).toContain('"passwordChanged":true');
    ...
    expect(h.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        afterJson: expect.objectContaining({
          profileFieldsChanged: ["avatarUrl", "gender", "birthday", "heightCm", "weightKg"],
        }),
      }),
    });
  });
  ...
  it.each([
    ...
    [{ newPassword: "1234567" }, "新密码须至少8位"],
  ])("rejects invalid App profile details: %j", async (profile, message) => {
    const h = updateHarness();
    await expect(
      h.service.updateMemberProfile(superAdmin, h.current()!.id, "invalid-profile", {
        nickname: "Synthetic member",
        mobile: "+8613812348888",
        email: "member@example.invalid",
        status: "ACTIVE",
  ...
  it("rejects invalid, duplicate, stale and protected member profile changes", async () => {
    ...
    await expect(
      invalid.service.updateMemberProfile(superAdmin, invalid.current()!.id, "r", {
        nickname: "",
        mobile: "13812348888",
        email: "bad",
        status: "ACTIVE",
    ...
    await expect(
      duplicate.service.updateMemberProfile(superAdmin, duplicate.current()!.id, "r", {
        nickname: "Synthetic member",
        mobile: "+8613812348888",
        email: "member@example.invalid",
        status: "ACTIVE",
    ...
    await expect(
      stale.service.updateMemberProfile(superAdmin, stale.current()!.id, "r", {
        nickname: "Synthetic member",
        mobile: "+8613812348888",
        email: "member@example.invalid",
        status: "ACTIVE",
    ...
    await expect(
      protectedMember.service.updateMemberProfile(superAdmin, protectedMember.current()!.id, "r", {
        nickname: "Synthetic member",
        mobile: "+8613812348888",
        email: "member@example.invalid",
        status: "ACTIVE",
  ...
  it("rejects missing contacts, stale pages, non-super-admins and domestic requests", async () => {
    ...
    const stale = updateHarness();
    await expect(
      stale.service.updateMemberVerification(superAdmin, stale.current()!.id, "r", {
        channel: "mobile",
        verified: true,
        expectedUpdatedAt: "2026-09-10T23:59:59.000Z",
      }),
    ).rejects.toThrow("已发生变化");
    ...
    const denied = updateHarness();
    await expect(
      denied.service.updateMemberVerification({ id: "operator", role: "APP_OPERATIONS", roles: ["APP_OPERATIONS"] }, denied.current()!.id, "r", {
        channel: "mobile",
        verified: true,
        expectedUpdatedAt: initialUpdatedAt.toISOString(),
      }),
    ...
    vi.stubEnv("APP_REALM", "domestic");
    await expect(
      denied.service.updateMemberVerification(superAdmin, denied.current()!.id, "r", {
        channel: "mobile",
        verified: true,
        expectedUpdatedAt: initialUpdatedAt.toISOString(),
      }),
