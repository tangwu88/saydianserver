...
describe("order list query compatibility", () => {
  it("keeps missing and old exact status requests, including existing case normalization", () => {
    ...
    expect(commerceOrderListFilter("WAITING_FULFILLMENT")).toEqual({
      status: "WAITING_FULFILLMENT",
    });
    expect(commerceOrderListFilter(" paid ")).toEqual({ status: "PAID" });
    expect(commerceOrderListFilter("AFTER_SALE")).toEqual({
      status: "AFTER_SALE",
    });
  });
  it.each([
    ["pending_shipment", { status: { in: ["PAID", "WAITING_FULFILLMENT"] } }],
    ["after_sales", { afterSales: { some: {} } }],
  ] as const)(
    "uses an actual database predicate for group %s",
    (group, expected) => {
  ...
  it.each([
    ...
    ["PAID", "after_sales"],
    [" ", undefined],
  ])("rejects invalid or ambiguous filters %j %j", (status, group) => {
  ...
  it("keeps ownership and queries actual after-sales independently of current order status", async () => {
    ...
    expect(
      (await store.listOrders("member", undefined, "pending_shipment")).map(
        (row) => row.id,
      ),
    ).toEqual(["paid", "waiting"]);
    expect(
      (await store.listOrders("member", undefined, "after_sales")).map(
        (row) => row.id,
      ),
    ).toEqual(["refunded", "rejected"]);
    expect(db.commerceOrder.findMany).toHaveBeenLastCalledWith(
      expect.objectContaining({
        where: { userId: "member", afterSales: { some: {} } },
      }),
    );
    expect(
      (await store.listOrders("member", "AFTER_SALE")).map((row) => row.id),
    ).toEqual(["status-only"]);
  });
  it("never treats a legacy status as evidence of actual after-sales; keeps normal legacy filters", async () => {
    const db = {
      legacyOrderProjection: { findMany: vi.fn().mockResolvedValue([]) },
    };
    ...
    expect(db.legacyOrderProjection.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          userId: "member",
    ...
    expect(db.legacyOrderProjection.findMany).toHaveBeenLastCalledWith(
      expect.objectContaining({
        where: { userId: "member", status: "AFTER_SALE" },
      }),
    );
  });
  it("both public controllers preserve group and status as separate optional parameters", async () => {
    ...
    await new CommerceController(commerce as any, {} as any).orders(
      user,
      undefined,
      "after_sales",
    );
    await new CommerceCompatibilityController(
      {} as any,
      commerce as any,
      {} as any,
      {} as any,
    ...
    expect(commerce.orders.mock.calls).toEqual([
      ...
      ["member", undefined, "pending_shipment"],
    ]);
  });
  it("internal commerce GET forwards group without filtering an already-loaded page", async () => {
    const store = { listOrders: vi.fn().mockResolvedValue([]) };
    await new CommerceService({} as any, store as any, {} as any).forUser(
      "member",
      "GET",
      "/orders?group=after_sales",
    );
    expect(store.listOrders).toHaveBeenCalledWith(
      "member",
      undefined,
