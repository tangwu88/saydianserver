...
商品、购物车、地址、订单、物流、售后、评价、优惠券和员工推广进入主系统。商品分为 ERP 与 LOCAL：ERP 的权威字段不得从后台覆盖；自建商品可管理 SKU、价格和本地库存。ERP 权威值与本地预占独立核算的完整验收仍见 [当前实施表](unification/implementation-status.md)。
...
- 仅有历史投影的旧订单 `readOnly=true`；正式目标是全部旧订单/售后/佣金/提现迁入运行模型并经接管门禁开放，不再永久只读。LEGACY_SYSTEM 所有权锁解除前不可发起新资金或履约请求；已有退款与转账必须核验原渠道单号，不重发。历史物流未完整迁移时不能把空数组解释为“没有物流”。
- 企业微信员工入口只接受允许域名上的 HTTPS 回调，签发独立员工会话；推广、奖金和赠券保留。提现运行模型支持申请审核和真实人工凭证记录，不代表已接入或自动执行打款；原提现在途记录须先核验原转账。赠券原始令牌只在创建时返回一次，数据库只保存哈希。
