...
4. 提交 `f0057ed6d131fd5d417a8e4ab907488a32e1a485` 推送成功，但 GitHub Actions `33980643757` 在没有启动任何 step 前失败；平台注解明确为账户近期付款失败或消费上限不足，`auto-deploy` 因此跳过。这不是代码测试失败。

## 未验收

- 待真实 TestFlight 或 App Store 地址取得后，通过后台校验后再开放 iPhone 按钮。
- Android 使用 QA 签名，尚需测试机在保留本地数据前提下完成升级签名兼容验收。
- 待 GitHub 账户结算或 Actions 消费上限恢复后，重跑 CI 与自动发布；页面的实际生产可用性不依赖该次滚动。
...
- 2026-09-05T17:21:38.6618520Z：pnpm api:docs:check，退出码 0。

- 2026-09-05T17:21:53.4376210Z：pnpm tools:test，退出码 0。

- 2026-09-05T17:22:14.1276370Z：pnpm typecheck，退出码 0。

- 2026-09-05T17:22:31.4802650Z：pnpm test，退出码 0。

- 2026-09-05T17:23:04.8292350Z：pnpm build，退出码 0。
