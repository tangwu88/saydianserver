...
- 国际源码检查点：`4634ba1`（63个明确列出的源码、测试、文档/部署模板文件；无构建产物）。随后在国际分支执行 `git merge --no-edit origin/main`，生成 `ceacc2bbb94843882b2d5f01fa76917c5b86ac40`；工具测试文件自动合并，无冲突。
- 合入 `5bf5ec6e3fc624587536a2a8e76d28a2a24b6c75` 后重新执行 `pnpm tools:test`：8项全部通过（含新增“HTTP fixture先验证OTP再注册”）；`pnpm test`：再次487通过、4个数据库测试跳过；`pnpm typecheck`：全部通过。最后再次fetch，确认origin/main仍为5bf5ec6，且为本国际分支祖先。
- `node deploy/global/check.mjs` 再次111结构检查通过；Docker Compose/Nginx运行、数据库E2E、真实供应商仍未验收。`git diff --check`通过，当前仅补本实施记录的本地提交，不push任何分支、不部署。
